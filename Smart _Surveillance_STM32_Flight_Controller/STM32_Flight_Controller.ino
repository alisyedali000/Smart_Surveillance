...
bool calibrate_compasss = false;
bool hover_throttle = false;
bool RTH = false;
...
int32_t serial_throttle = 1000;                                       //for channel_3 
...
void loop() {
  ...
  //  channel_3 = 1000;
    ...
    if (number_used_sats > 5) {
      if (home_point_recorded == 1 && RTH == true)channel_5 = 2000;
      else channel_5 = 1750;
    }
   // else channel_5 = 1100;    /// 1500 here for altitude hold mode
  }
  ...
  read_compass();                                                                  //Read and calculate the compass data.
  si_translate_bytes();
  read_start_stop();                                                               //Read serial data from ground station to turn on/off quadcopter.
  ...
  channel_3 = serial_throttle;
  channel_5 = serial_flight_mode;
